...
export class Line extends Base {
    ...
    async animateCoords(
        ...
        await this.animateUpdateCoords(
            updateCoordsWrapper(option),
            time,
            easing,
        );
    }
    async animateUpdateCoords(
        updateCoords: LineUpdater,
        time = 400,
        easing = sine,
    ): Promise<void> {
        ...
        this.updateCoords = function () {
            ...
            const { x0: oldX0, y0: oldY0, x1: oldX1, y1: oldY1 } = this;
            updateCoords.call(this);
            const { x0: newX0, y0: newY0, x1: newX1, y1: newY1 } = this;
